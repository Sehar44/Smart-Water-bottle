...
//for
  bluetooth - create an object called BTserial, with RX pin at 3 and TX pin at 2
SoftwareSerial
  BTserial(3,2); // RX | TX
...
float
  R = 0;//resistance between the 2 wires
float r = 0;//resistivity 
float L
  = 0.06;//distance between the wires in m
double A = 0.000154;//area of cross
  section of wire in m^2

float C = 0;//conductivity in S/m
float Cm = 0;//conductivity
  in mS/cm
...
//we will use this formula to get the resistivity
  after using ohm's law -> R = r L/A => r = R A/L

//creating lcd object from
...
//
  ...
  above
    ...
    //display corresponding colours on rgb led according
  to the analog read
     if( reads < 600 )
  {
      if (reads <= 300){
...
//send data to Ardutooth app on mobile phone
  through bluetooth
BTserial.print(C);
BTserial.print(",");
BTserial.print(TDS);
